...
final class ListProductPresenter implements ListProductContract.Presenter {
    ...
    @Override
    public void getListAllProductByFillter(String priceSort, int priceFrom, int priceTo) {
        Subscription subscription =
                mProductRepository.getListAllProductByFillter(priceSort, priceFrom, priceTo)
                        .subscribeOn(Schedulers.io())
                        .doOnSubscribe(new Action0() {
                            @Override
                            public void call() {
